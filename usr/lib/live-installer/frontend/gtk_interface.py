...
class InstallerWindow:
    ...
    def __init__(self, fullscreen=False):
        ...
        self.wizard_pages[self.PAGE_LANGUAGE] = WizardPage(_("Linguagem"), "locales.png")
        self.wizard_pages[self.PAGE_TIMEZONE] = WizardPage(_("Timezone"), "time.png")
        self.wizard_pages[self.PAGE_KEYBOARD] = WizardPage(_("Layout do teclado"), "keyboard.png")
        self.wizard_pages[self.PAGE_USER] = WizardPage(_("Informações do usuário"), "user.png")
        self.wizard_pages[self.PAGE_PARTITIONS] = WizardPage(_("Particionamento"), "hdd.svg")
        self.wizard_pages[self.PAGE_CUSTOMWARNING] = WizardPage(_("Please make sure you wish to manage partitions manually"), "hdd.svg")
        self.wizard_pages[self.PAGE_ADVANCED] = WizardPage(_("Opcoes avancadas"), "advanced.png")
        self.wizard_pages[self.PAGE_OVERVIEW] = WizardPage(_("Resumo"), "summary.png")
        self.wizard_pages[self.PAGE_INSTALL] = WizardPage(_("Instalando Emmi Linux..."), "install.png")
        ...
        col = gtk.TreeViewColumn(_("Linguagem"), ren, text=0)
        col.set_sort_column_id(0)
        self.wTree.get_widget("treeview_language_list").append_column(col)
        col = gtk.TreeViewColumn(_("País"), ren, text=1)
        ...
        self.wTree.get_widget("button_timezones").set_label(_('Seleciona a timezone'))
        self.wTree.get_widget("event_timezones").connect('button-release-event', timezones.cb_map_clicked, model)

        # partitions
        self.wTree.get_widget("button_edit").set_label(_("Editar partiçoes"))
        self.wTree.get_widget("button_refresh").set_label(_("Atualizar"))
        self.wTree.get_widget("button_custommount").set_label(_("Modo avançado"))
    ...
    def i18n(self):
        ...
        self.wTree.get_widget("label_your_name_help").set_markup("<span fgcolor='#3C3C3C'><sub><i>%s</i></sub></span>" % _("Digite seu nome completo."))
        self.wTree.get_widget("label_username").set_markup("<b>%s</b>" % _("Nome de usuario"))
        self.wTree.get_widget("label_username_help").set_markup("<span fgcolor='#3C3C3C'><sub><i>%s</i></sub></span>" % _("Esse é o nome usado para logar no seu computador."))
        ...
        self.face_button.set_tooltip_text(_("Clique para trocar a imagem"))
        self.face_photo_menuitem.set_label(_("Tirar uma foto..."))
        self.face_browse_menuitem.set_label(_("Navegar por mais imagens..."))

        # timezones
        self.wTree.get_widget("label_timezones").set_label(_("Selecione timezone:"))
        ...
        self.wTree.get_widget("label_grub_help").set_label(_("GRUB é o bootloader usado para carregar o kernel Linux."))
        
        # keyboard page
        self.wTree.get_widget("label_test_kb").set_label(_("Use essa caixa para testar seu layout do teclado."))
        self.wTree.get_widget("label_kb_model").set_label(_("Modelo"))        
        ...
        self.column11.set_title(_("Variante")) 
    ...
    def quit_cb(self, widget, data=None):
        if QuestionDialog(_("Quit?"), _("Você realmente quer sair do instalador?")):
            gtk.main_quit()
            return False
        else:
            return True
    ...
    def assign_password(self, widget):
        ...
        if(self.setup.password1 != self.setup.password2):
            ...
            self.wTree.get_widget("label_mismatch").set_label(_("Senhas não conferem."))
        else:
            self.wTree.get_widget("image_mismatch").set_from_stock(gtk.STOCK_OK, gtk.ICON_SIZE_BUTTON)            
            self.wTree.get_widget("label_mismatch").set_label(_("Senhas conferem."))
        self.setup.print_setup()
        ...
        if(not goback):
            if(sel == self.PAGE_LANGUAGE):
                ...
                if(self.setup.real_name is None or self.setup.real_name == ""):
                    errorFound = True
                    errorMessage = _("Por favor digite seu nome completo.")
                elif(self.setup.username is None or self.setup.username == ""):
                    errorFound = True
                    errorMessage = _("Por favor digite seu nome de usuário.")
                elif(self.setup.password1 is None or self.setup.password1 == ""):
                    errorFound = True
                    errorMessage = _("Por favor digite a senha do usuário.")
                elif(self.setup.password1 != self.setup.password2):
                    errorFound = True
                    errorMessage = _("As senhas não conferem.")
                elif(self.setup.hostname is None or self.setup.hostname == ""):
                    errorFound = True
                    errorMessage = _("Por favor digite o hostname.")
                else:
                    for char in self.setup.username:
                        if(char.isupper()):
                            errorFound = True
                            errorMessage = _("Nome de usuário deve ser em letra minúsculas.")
                            break
                        elif(char.isspace()):
                            errorFound = True
                            errorMessage = _("Seu usuário não pode conter espaços em branco.")
                    
                    for char in self.setup.hostname:
                        if(char.isupper()):
                            errorFound = True
                            errorMessage = _("Hostname deve ser em letras minúsculas.")
                            break
                        elif(char.isspace()):
                            errorFound = True
                            errorMessage = _("Hostname não pode conter espaços em branco.")
                    
                if (errorFound):
                    WarningDialog(_("Assistente de instalação"), errorMessage)
                else:
                ...
                if not found_root_partition:
                    ErrorDialog(_("Installation Tool"), "<b>%s</b>" % _("Por favor selecione a partição raiz (/)."), _("A partição raiz é necessária para instalar o Emmi Linux.\n\n - Ponto de montagem: /\n - Tamanho recomendado: 30GB\n - Sistema de arquivos recomendado: ext4\n "))
                    return
    ...
    def show_overview(self):
        ...
        top = model.append(None, (_("Localização"),))
        model.append(top, (_("Língua: ") + bold(self.setup.language),))
        model.append(top, (_("Timezone: ") + bold(self.setup.timezone),))
        model.append(top, (_("Layout teclado: ") +
        ...
        top = model.append(None, (_("Opções do usuário"),))
        model.append(top, (_("Nome real: ") + bold(self.setup.real_name),))
        model.append(top, (_("Username: ") + bold(self.setup.username),))
        model.append(top, (_("Login automático: ") + bold(_("enabled") if self.setup.autologin else _("disabled")),))
        top = model.append(None, (_("Opções do sistema"),))
        model.append(top, (_("Hostname: ") + bold(self.setup.hostname),))
        top = model.append(None, (_("Opções do sistema de arquivos"),))
        model.append(top, (bold(_("Instalar bootloader em %s") % self.setup.grub_device) if self.setup.grub_device else _("Bootloader não será instalado"),))
        if self.setup.skip_mount:
            model.append(top, (bold(_("Use already-mounted /target.")),))
            return
        for p in self.setup.partitions:
            if p.format_as:
                model.append(top, (bold(_("Formatar %(path)s para %(filesystem)s") % {'path':p.partition.path, 'filesystem':p.format_as}),))
        for p in self.setup.partitions:
            if p.mount_as:
                model.append(top, (bold(_("Montar %(path)s como %(mount)s") % {'path': p.partition.path, 'mount':p.mount_as}),))
    ...
    def do_install(self):        
        ...
        try:
            ...
            with gtk.gdk.lock:
                ErrorDialog(_("Erro na instalação"), str(detail1))

        if self.critical_error_happened:
            with gtk.gdk.lock:
                ErrorDialog(_("Erro na instalação"), self.critical_error_message)
            do_try_finish_install = False
        ...
        if do_try_finish_install:
            ...
            with gtk.gdk.lock:
                if self.critical_error_happened:
                    ErrorDialog(_("Instalação concluida"), self.critical_error_message)
                else:
                    reboot = QuestionDialog(_("Instalação finalizada"), _("A instalação está completa. Deseja reiniciar seu computador para usar seu novo sistema?"))
                    if reboot:
                        os.system('reboot')
